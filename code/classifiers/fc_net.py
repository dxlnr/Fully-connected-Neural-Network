...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores_hidden, cache_hidden = affine_relu_forward(X, W1, b1)
        scores, cache = affine_forward(scores_hidden, W2, b2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(0,self.num_layers):
            ...
            gamma = 'gamma' + str(i+1) 
            beta = 'beta' + str(i+1)
            batchnorm_name = 'batchnorm' + str(i+1)
            dropout_name = 'dropout' + str(i+1)
            cache_name = 'c' + str(i+1)
            
            
            if i == (self.num_layers-1):
                scores, cache = affine_forward(scores, self.params[W], self.params[b])
                
            else:
                if self.use_batchnorm:
                    scores, self.batchnorm_cache[batchnorm_name] = batchnorm_forward(scores, self.params[gamma], self.params[beta], self.bn_params[i])
        ...
        for i in range(self.num_layers,0,-1):
            ...
            if i == self.num_layers:
                dscores, grads[W], grads[b] = affine_backward(dscores, self.cache[cache_name])
            else:
                if self.use_dropout:
                    dscores = dropout_backward(dscores, self.dropout_cache[dropout_name])
                    
                dscores, grads[W], grads[b] = affine_relu_backward(dscores, self.cache[cache_name])
                
                if self.use_batchnorm:
                    dscores, grads[gamma], grads[beta] = batchnorm_backward(dscores, self.batchnorm_cache[batchnorm_name])
